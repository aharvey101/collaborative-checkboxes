...
thread_local! {
    static PIXEL_RENDER_SCHEDULED: std::cell::RefCell<bool> = const { std::cell::RefCell::new(false) };
}

/// Apply pixel updates from the worker to loaded_chunks.
/// bytes: packed [N × 16 bytes: chunk_id(8) + cell_offset(4) + r + g + b + checked]
pub fn apply_pixel_updates(bytes: &[u8], count: usize) {
    TEST_STATE.with(|s| {
        let state = s.borrow();
        ...
        // Coalesce renders
        let already_scheduled = PIXEL_RENDER_SCHEDULED.with(|f| {
            let was = *f.borrow();
            *f.borrow_mut() = true;
            was
        });
        ...
        if !already_scheduled {
            ...
            let closure = wasm_bindgen::closure::Closure::once(Box::new(move || {
                PIXEL_RENDER_SCHEDULED.with(|f| *f.borrow_mut() = false);
